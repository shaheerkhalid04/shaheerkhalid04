...
| **[Image Generator](https://github.com/shaheerkhalid04/image-generator)** | Describe a picture, get it back in the chat, with no API key or account anywhere. Built to learn Chainlit properly: the whole app is decorators and event callbacks, with no render loop. Gemini's image models turn out to be paid tier only, so it renders keyless instead, and it pins the seed so "render larger" returns the same picture with more pixels rather than a different one. | `Chainlit` `Python` `asyncio` |
